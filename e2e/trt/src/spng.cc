...
#define SPNG_READ_SIZE 8192
    ...
    #if (__GNUC__ >= 7 && !defined(__clang__)) || defined(__INTEL_COMPILER) && defined(SPNG_X86)
...
static int read_chunks_before_idat(spng_ctx *ctx)
{
    ...
    while( !(ret = read_header(ctx, &discard)))
    {
        ...
        else if(!memcmp(chunk.type, type_splt, 4))
        {
            ...
            {
                ctx->n_splt = 1;
                ctx->splt_list = (spng_splt *) spng__calloc(ctx, 1, sizeof(struct spng_splt));
            ...
            char *keyword_nul = (char *) memchr(data, '\0', keyword_len);
            ...
            ctx->splt_list[i].entries = (spng_splt_entry *) spng__malloc(ctx, sizeof(struct spng_splt_entry) * ctx->splt_list[i].n_entries);
...
int spng_decode_image(spng_ctx *ctx, unsigned char *out, size_t out_size, int fmt, int flags)
{
    ...
    unsigned char *scanline = (unsigned char *) spng__malloc(ctx, scanline_width);
    unsigned char *prev_scanline = (unsigned char *) spng__malloc(ctx, scanline_width);
    ...
    if(apply_gamma)
    {
        ...
        else /* SPNG_FMT_RGBA16 */
        {
            ...
            ctx->gamma_lut = (uint16_t *) spng__malloc(ctx, lut_entries * sizeof(uint16_t));
...
int spng_set_png_buffer(spng_ctx *ctx, const void *buf, size_t size)
{
    ...
    ctx->png_buf = (const unsigned char*) buf;
    ctx->data_size = size;
    ctx->bytes_left = size;
